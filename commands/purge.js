exports.run = function(client, message, args) {
  ...
  if (isNaN(messageCount) && messageCount) return message.channel.send(`${messageCount} is not a number`);
  if (!messageCount) {
    ...
    return message.channel.sendMessage(`${messageCount} is too big (max is 50)`);
  } else if (messageCount > 2) {
    return message.channel.sendMessage(`${messageCount} is too small (minium is 2)`);
  }
};
